...
class CenterHead(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            target_dict_iou = self.assign_iou_target() # add by Huang
            self.forward_ret_dict.update(targets_dict)
            self.forward_ret_dict.update(target_dict_iou) # add by Huang

        if not self.training or self.predict_boxes_when_training:
            batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
                batch_size=data_dict['batch_size'],
                cls_preds=cls_preds, box_preds=box_preds, iou_preds=iou_preds, dir_cls_preds=None # edit by Huang
            )
            data_dict['batch_cls_preds'] = batch_cls_preds
            data_dict['batch_box_preds'] = batch_box_preds
            data_dict['cls_preds_normalized'] = True   #edit by Huang
    ...
    def assign_iou_target(self):
        ...
        batch_xs = xs.view(batch, -1, 1) + batch_reg[:, :, 0:1]
        batch_ys = ys.view(batch, -1, 1) + batch_reg[:, :, 1:2]

        batch_xs = batch_xs * self.target_cfg.OUT_SIZE_FACTOR * self.target_cfg.VOXEL_SIZE[0] + self.point_cloud_range[0]
        batch_ys = batch_ys * self.target_cfg.OUT_SIZE_FACTOR * self.target_cfg.VOXEL_SIZE[1] + self.point_cloud_range[1]
        batch_rot = torch.atan2(batch_rots, batch_rotc)
        # B x (H x W) x 7
        batch_box_preds = torch.cat([batch_xs, batch_ys, batch_hei, batch_dim, batch_rot], dim=2)
        # B x (max_obj) x 7
        batch_pred = self._gather_feat(batch_box_preds,self.forward_ret_dict['inds'][0])

        target_box = self.forward_ret_dict['anno_boxes'][0] #B x max_obj x 8
        ...
        ys, xs = torch.meshgrid([torch.arange(0, H), torch.arange(0, W)])
        ys = ys.view(1, 1, H, W).repeat(batch, 1, 1, 1).to(cls_preds.device)
        xs = xs.view(1, 1, H, W).repeat(batch, 1, 1, 1).to(cls_preds.device)

        xs = xs.permute(0, 2, 3, 1).contiguous()
        xs = xs.view(xs.size(0), -1, xs.size(3))

        ys = ys.permute(0, 2, 3, 1).contiguous()
        ys = ys.view(ys.size(0), -1, ys.size(3))

        batch_gt_xs = self._gather_feat(xs, self.forward_ret_dict['inds'][0]) + batch_gt_reg[..., 0:1]
        batch_gt_ys = self._gather_feat(ys, self.forward_ret_dict['inds'][0]) + batch_gt_reg[..., 1:2]
        batch_gt_xs = batch_gt_xs * self.target_cfg.OUT_SIZE_FACTOR * self.target_cfg.VOXEL_SIZE[0] + self.point_cloud_range[0]
        batch_gt_ys = batch_gt_ys * self.target_cfg.OUT_SIZE_FACTOR * self.target_cfg.VOXEL_SIZE[1] + self.point_cloud_range[1]
        
        batch_box_targets = torch.cat([batch_gt_xs, batch_gt_ys, batch_gt_hei, batch_gt_dim, batch_gt_rot], dim=-1)

         # pass to iou3d to calculate
        iou_targets = iou3d_nms_utils.boxes_iou3d_gpu(batch_pred.reshape(-1, 7), batch_box_targets.reshape(-1, 7))[range(
            batch_pred.reshape(-1, 7).shape[0]), range(batch_box_targets.reshape(-1, 7).shape[0])]

        all_targets = {"iou_target": iou_targets.reshape(batch,-1,1)}
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        #add by Huang
        batch_iou_preds = iou_preds.view(batch, H*W, -1)
        batch_iou_preds = torch.clamp(batch_iou_preds,min=0,max=1)

        batch_cls_preds = cls_preds.view(batch, H*W, -1)
        batch_cls_preds = clip_sigmoid(batch_cls_preds)
        batch_cls_preds = batch_cls_preds * torch.pow(batch_iou_preds,self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['cf_weight'])
    ...
    '''
        compare the predicted iou and the target iou calculate the loss
    '''
    #add by Huang
    def get_iou_layer_loss(self):
        inds, masks =  self.forward_ret_dict['inds'][0], self.forward_ret_dict['masks'][0]

        ind = inds
        num = masks.float().sum()

        iou_pred = self.forward_ret_dict['iou_preds']
        iou_pred = iou_pred.view(iou_pred.size(0), -1, iou_pred.size(3))
        iou_pred = self._gather_feat(iou_pred, ind)

        iou_target = self.forward_ret_dict['iou_target']
        
        mask = masks.unsqueeze(2).expand_as(iou_target).float()

        isnotnan = (~torch.isnan(iou_target)).float()
        mask *= isnotnan

        iou_loss = l1_loss(
            iou_pred*mask, iou_target*mask, avg_factor=(num + 1e-4))
